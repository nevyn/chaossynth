...
- [ ] **B1 — headless sclang will likely crash-loop on the Pi.** Debian's sclang
...
- [ ] **B2 — master-volume doc contradiction.** patch-design.md (authoritative,
...
- [ ] **B3 — "remap live, no reflash" vs the overlayfs double-reboot.** README.md
...
- [ ] Evict pot-3 volume placeholder per patch-design (retune smoke-test's
  "pot 3 drives the volume" check to pot 3's real role).
- [ ] `synth/tools/smoke-test.sh:31` — cleanup `pkill -x scsynth` kills ANY
  scsynth on the machine, including a sibling session's. Scope it to its own
  child (process group / `pgrep -P`).
- [ ] Never-boots wedge: if `s.waitForBoot` gives up, sclang idles forever
  without `CHAOSSYNTH READY` and systemd never restarts it. Add a boot timeout
  → exit 1.
- [ ] Drone-on-boot: engine waits `CHAOS_IDLE_TIMEOUT` (90 s) after start before
...
- [ ] patch-design.md: one sentence defining how special buttons
  ...
  branch, unreferenced.)
- [ ] Sample spec drift: amen bank is 20 stereo WAVs; patch-design says mono.
...
- [ ] jackd under systemd: add `JACK_NO_AUDIO_RESERVATION=1`, playback-only
  `-P`, and `LimitRTPRIO=95` + `LimitMEMLOCK=infinity` to chaossynth.service
  (RT scheduling is otherwise denied to services; xrun risk).
- [ ] Add `overlayroot` to provision's apt line — Bookworm installs it lazily on
  first `overlayfs.sh on`, which would otherwise need working internet at
  lockdown time.
- [ ] prepare-sd.sh: comment/fallback for stock-macOS LibreSSL lacking
  `openssl passwd -6` (works on this Mac via Homebrew; fails loudly elsewhere).

## Needs Nevyn

- [ ] **Contract amendments (frozen doc, your sign-off):** CC 123 is sent on
...
- [ ] **git-lfs for samples** — your in-session wish, recorded nowhere; ~13 MB
  of WAV blobs in plain git and more banks coming. Decide before bank #2.
